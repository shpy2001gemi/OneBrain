...
| `TX-SOURCE-001` | Authenticated source-capture intent committed before Vault-encrypted staging and canonical/Vault reconciliation | Node-owned source-capture intent beneath the active `DatasetPathResolver`; exact UTF-8 exists only in zeroizing memory or Vault-key-encrypted staging | Bind the proven source record, quarantine an unreferenced stage, or expose `SourceCaptureIncomplete` | Journal metadata authenticates subject, canonical digest, exact source digest, staging ID, target Vault record and dataset generation; no plaintext journal, reconstructed prose, cross-generation binding or false retriever acknowledgement | Exact five-phase child-process kill/reopen matrix plus wrong-key, ciphertext-tamper, orphan cleanup and zeroization oracles |
