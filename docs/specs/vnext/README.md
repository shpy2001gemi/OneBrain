...
| `BASE-V1` | [Base v1 Authority and Recovery Profile](BASE_V1_AUTHORITY_AND_RECOVERY_PROFILE.md) | Frozen | Canonical/legacy authority, encrypted recovery, archive scope, signer separation, Registry fail-closed states, default-off networking and immutable-history delete semantics. |
...
| `DR-P3.1` | [vNext REST API Profile v1](VNEXT_REST_API_PROFILE_V1.md) | Complete | Implements all twelve reserved REST routes through the node-owned service façade with private preparation capabilities, exact consent, opaque pagination, honest status/coverage and fail-closed proposal/PoMV projections. |
| `DR-P3.2` | [vNext Private WebSocket Profile v1](VNEXT_PRIVATE_WEBSOCKET_PROFILE_V1.md) | Complete | Bearer-minted one-use tickets, immutable topic scope, exact-client bounded queues, private-field suppression, honest publication/view/lane events and slow-client isolation. |
| `DR-P3.3` | [vNext CLI Profile v1](VNEXT_CLI_PROFILE_V1.md) | Complete | Eleven additive authenticated API commands for private Needs, explicit Public Use, evidence views and runtime status, with exact typed confirmation, no `--yes`, scope-honest output and double-opt-in development Feed signer. |
| `DR-P3.4` | [vNext Desktop/Web UX Profile v1](VNEXT_DESKTOP_WEB_UX_PROFILE_V1.md) | Complete | Separate Local KQL/one-hop and legacy/vNext PoMV surfaces, exact-confirmation Public Use wizard, honest quarantine/outbox/conflict/status rendering, cross-language receipt vectors and graceful Desktop quit/restart. |
...
| `M5-00` | [Distributed Runtime Hardening Baseline v1](DISTRIBUTED_RUNTIME_HARDENING_BASELINE_V1.md) and [Transaction Boundary Inventory v1](DISTRIBUTED_RUNTIME_TRANSACTION_BOUNDARY_INVENTORY_V1.md) | Contract frozen | Machine-checked runtime path trigger, finite real-QUIC timeout, acceptance-step inventory, 13 stable transaction IDs, five failpoint phases and 11-field invariant oracle. |
| `M5-01` | [Unified Resource Admission and Fairness v1](UNIFIED_RESOURCE_ADMISSION_AND_FAIRNESS_V1.md) | Frozen | One ordered stream-to-application admission controller, pre-allocation lane caps, global/IP/NodeID quotas, bounded durable stores and prefix pages, plus restart-safe fair outbox terminal handling. |
...
| `CR-OPS-01` | [Concept Registry Operations Profile v1](CONCEPT_REGISTRY_OPERATIONS_PROFILE_V1.md) | Foundation implemented | Signed exact-file releases with source/SBOM provenance, capacity admission, append-only atomic activation/rollback, truncated-index and disk-shortage drills, required-mode fail-closed runtime selection, and operator CLI. |
| `CR-PROD-01` | [Concept Registry Production Qualification Profile v1](CONCEPT_REGISTRY_PRODUCTION_QUALIFICATION_PROFILE_V1.md) | Contract frozen; evidence open | Exact five-payload root, 2.2 GB-class bounds, cold/low-RAM/SSD/HDD budgets, Linux reference identity, owner-approved Registry signer/trust policy, closed release context, process-kill/live-reader/rollback/CCID gates, and a complete signed release cycle without claiming measured completion. |
