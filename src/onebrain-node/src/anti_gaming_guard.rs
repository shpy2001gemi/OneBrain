...
impl AntiGamingGuard {
    /// Create a new guard for a Leaf-tier node (epoch 0).
    pub fn new() -> Self {
        Self {
    ...
    pub fn check_rate_limit(&self) -> Result<(), String> {
        ...
        if self.rate_tracker.check_ku_rate(now, self.node_tier) {
            Ok(())
        } else {
            ...
            let max_per_hour = match self.node_tier {
                NodeTier::Leaf => 1,
                NodeTier::Contributor => 5,
                _ => 10,
            };
            Err(format!(
