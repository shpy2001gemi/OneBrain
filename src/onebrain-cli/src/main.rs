...
async fn main() {
    ...
    match args.command {
        Commands::Start { name, port, data_dir, ollama_url, model, seeds, api, api_port, api_token, web_dir } => {
            let config = NodeConfig { name, port, data_dir, ollama_url, model, seeds };
            ...
            let mut node = match OneBrainNode::new(config.clone()).await {
                Ok(n) => n,
                Err(e) => { eprintln!("Failed to start node: {}", e); std::process::exit(1); }
            };
            ...
            match node.start_network().await {
                ...
                Err(e) => eprintln!("  \u{26a0} Network start failed: {} (continuing without networking)", e),
            }
            ...
            for peer_addr in &mdns.discovered_peers { println!("    LAN peer: {}", peer_addr); }
            ...
            match seed.connect().await {
                Ok(_) => {
                    if let Some(stream) = seed.stream() {
                        SeedClient::run_background(stream, peer_id.clone(), node.event_tx.clone()).await;
                    }
                    match seed.get_peers().await {
                        Ok(peers) => {
                            println!("  \u{2713} Found {} peer(s) online", peers.len());
                            for p in &peers {
                                let short_id = if p.peer_id.len() >= 8 { &p.peer_id[..8] } else { &p.peer_id };
            ...
            if memory.peer_count() > 0 {
                println!("  \u{2713} Remembered {} peer(s) from last session", memory.peer_count());
            }
            ...
            if api {
                ...
                let mut api_server = ApiServer::with_shared_node(
                    shared_node.clone(),
                    api_token.clone(),
                    api_port,
                );
