...
mod benchmark {
    ...
    fn codon(concept_id: ConceptId, role: RoleId) -> Codon {
        Codon { concept_id, role, qualifiers: vec![] }
    }
    ...
    fn test_roundtrip_all_gene_types() {
        ...
        let test_cases: Vec<(&str, KnowledgeUnit)> = vec![
            ...
            ("Procedure", make_ku(
                ...
                Gene::Procedure {
                    steps: vec![
                        ProcedureStep { ord: 1, act: C_GRIND, pre: vec![], tgt: C_COFFEE, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 2, act: C_HEAT, pre: vec![], tgt: C_WATER, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 3, act: C_POUR, pre: vec![], tgt: C_WATER, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 4, act: C_BREW, pre: vec![], tgt: C_COFFEE, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 5, act: C_FILTER, pre: vec![], tgt: C_COFFEE, tools: vec![], eff: vec![], warn: vec![] },
                    ],
            ...
            ("Experience", make_ku(
                ...
                Gene::Experience {
                    ...
                    affect: Affect { v: 8500, a: 3000, d: 6000 },
                    canonical: Some(CanonicalText {
                        lang: 1,
                        text: b"Sunset at the beach was breathtaking".to_vec(),
                    }),
                    perspective: Some(Perspective { expertise: 0, perspective_type: 1 }),
                },
            )),
            ...
            ("Creative", make_ku(
                ...
                Gene::Creative {
                    steps: vec![
                        ProcedureStep { ord: 1, act: C_HEAT, pre: vec![], tgt: C_WATER, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 2, act: C_COOK, pre: vec![], tgt: C_INGREDIENT, tools: vec![], eff: vec![], warn: vec![] },
                        ProcedureStep { ord: 3, act: C_SERVE, pre: vec![], tgt: C_RECIPE, tools: vec![], eff: vec![], warn: vec![] },
                    ],
            ...
            ("MediaExperience", make_ku(
                ...
                Gene::MediaExperience {
                    ...
                    media_type: 0,  // FILM
                    rating: 92,
                    affect: Affect { v: 8000, a: 7500, d: 5000 },
            ...
            ("Testimony", make_ku(
                ...
                Gene::Testimony {
                    triples: vec![
                        Triple { subject: C_WITNESS, predicate: C_SEE, object: C_LIGHT },
                    ],
        ...
        for (name, ku) in &test_cases {
            ...
            assert!(gene_match, "{}: gene content mismatch after roundtrip", name);
        }
        ...
        assert!(all_passed, "Not all gene types passed round-trip validation");
    }
    ...
    fn test_size_benchmark() {
        ...
        let ku_minimal = KnowledgeUnit {
            ...
            bonds: vec![],
            gene: Gene::Fact {
                triples: vec![Triple { subject: C_WATER, predicate: C_IS, object: C_TRUE }],
                certainty: 5000,
                evidence: vec![],
            },
            flags: HeaderFlags::default(),
        ...
        let ku_full = KnowledgeUnit {
            ...
            gene: Gene::Fact {
                triples: vec![
                    Triple { subject: C_WATER, predicate: C_BOILING_POINT, object: C_CELSIUS },
                    Triple { subject: C_HEAT, predicate: C_IS, object: C_ENERGY },
                ],
                certainty: 9900,
        ...
        for (label, ku) in [("Minimal", &ku_minimal), ("Typical", &ku_typical), ("Full", &ku_full)] {
            let wire = encode_knowledge_unit(ku).unwrap();
            let total = wire.len();
            let header = 6;
            let crc = 4;
        ...
        println!("\n  Size Assertions:");
        println!("    Minimal ({:>4}B) < 200B: {}", minimal_size,
            if minimal_size < 200 { "PASS ✓" } else { "FAIL ✗" });
        println!("    Typical ({:>4}B) < 500B: {}", typical_size,
            if typical_size < 500 { "PASS ✓" } else { "FAIL ✗" });
        println!("    Full    ({:>4}B) < 1500B: {}", full_size,
            if full_size < 1500 { "PASS ✓" } else { "FAIL ✗" });

        assert!(minimal_size < 200, "Minimal KU ({} bytes) should be < 200B", minimal_size);
        assert!(typical_size < 500, "Typical KU ({} bytes) should be < 500B", typical_size);
        assert!(full_size < 1500, "Full KU ({} bytes) should be < 1500B (512B embed + 3 bonds + trust)", full_size);
    ...
    fn test_error_handling_comprehensive() {
        ...
        // 2. InvalidMagic — wrong first 2 bytes
        {
            let ku = make_ku(
                ...
                vec![],
                Gene::Fact { triples: vec![], certainty: 0, evidence: vec![] },
            );
        ...
        // 8. InvalidData — EXTENDED gene type with empty payload
        {
            ...
            match decode_knowledge_unit(&wire).unwrap_err() {
                KuError::InvalidData(msg) => {
                    assert!(msg.contains("EXTENDED"), "Message should mention EXTENDED: {}", msg);
    ...
    fn test_varint_boundaries() {
        ...
        println!("\n  Max reachable value (u32::MAX + 2,113,664 = {}): {} bytes ✓",
            max_reachable, encoded.len());

        // Varint tier capacity summary
        println!("\n  --- Varint Tier Capacities ---");
        println!("    Tier 0 (1B): 0–127            ({} values)", 128);
        println!("    Tier 1 (2B): 128–16,511       ({} values)", 16_511 - 128 + 1);
        println!("    Tier 2 (3B): 16,512–2,113,663 ({} values)", 2_113_663 - 16_512 + 1);
        println!("    Tier 3 (4B): 2,113,664–270,549,119 ({} values)", 270_549_119u64 - 2_113_664 + 1);
    ...
    fn test_wire_format_compliance() {
        ...
        println!("  Byte 0-1 MAGIC:       0x{:02X}{:02X} ('{}{}') ✓",
        ...
        println!("  Byte 3   FLAGS:       0x{:02X} (gene_base={}, ecc={}, enc={}) ✓",
        ...
        println!("  Byte 4-7 PAYLOAD_LEN: {} bytes (0x{:02X}{:02X}{:02X}{:02X}) ✓",
        ...
        assert!(wire.len() >= payload_end + 4,
            "Wire too short: {} < {}", wire.len(), payload_end + 4);
        println!("  Byte 8..{}: CBOR payload ({} bytes) ✓",
            payload_end - 1, payload_len);
        ...
        println!("  Byte {}..{}: CRC-32 = 0x{:08X} ✓",
            crc_offset, crc_offset + 3, stored_crc);
        ...
        println!("    MAGIC(2B) + VERSION(1B) + FLAGS(1B) + LEN(4B) + PAYLOAD({}B) + CRC32(4B) = {}B",
            payload_len, total);
    ...
    fn test_print_full_report() {
        ...
        let genes: Vec<(&str, Gene)> = vec![
            ("Fact", Gene::Fact {
                ...
                certainty: 9000, evidence: vec![],
            }),
            ("Procedure", Gene::Procedure {
                steps: vec![ProcedureStep { ord: 1, act: 1, pre: vec![], tgt: 2, tools: vec![], eff: vec![], warn: vec![] }],
                total_time: Some(300), difficulty: 2, tools_req: vec![],
            }),
            ("Experience", Gene::Experience {
                scene: vec![codon(1, RoleId::Agent)],
                affect: Affect { v: 5000, a: 5000, d: 5000 },
                canonical: None, perspective: None,
            }),
            ("Creative", Gene::Creative {
                steps: vec![], cultural_context: vec![], origin_story: None,
            }),
            ("MediaExperience", Gene::MediaExperience {
                id_sys: 1, ext_id: b"tt1234567".to_vec(), media_type: 0,
                rating: 85, affect: Affect { v: 7000, a: 6000, d: 5000 }, spoiler_level: 0,
            }),
            ("Testimony", Gene::Testimony {
                triples: vec![Triple { subject: 100, predicate: 200, object: 300 }],
                claim_type: 1, extraordinary: 0, witness_count: 1,
                proximity: 0, verification_status: 0,
            }),
            ("Formal", Gene::Formal {
                domain: 0, notation_format: 0, notation_source: b"E=mc^2".to_vec(),
                statement_type: 5, verification_status: 2,
            }),
            ("Hypothesis", Gene::Hypothesis {
                base_type: 0, body_codons: vec![codon(42, RoleId::Agent)],
                maturity_level: 2, confidence: 5000, completeness: 3000, falsifiable: true,
            }),
            ("Narrative", Gene::Narrative {
                narrative_type: 0, origin_culture: vec![5000], era: 6,
                function: 1, sacred: false, moral: vec![], canonical: None,
            }),
            ("Sensory", Gene::Sensory {
                modality: 0, property: 1000, feature: 2000,
                result_codons: vec![], sensor_type: 0, quality: 0,
            }),
        ];
