...
impl fmt::Display for KuError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            KuError::CrcMismatch { stored, computed } => write!(f, "CRC mismatch: stored={:08X}, computed={:08X}", stored, computed),
            KuError::VarintTruncated { needed, got } => write!(f, "Varint truncated: needed {} bytes, got {}", needed, got),
            KuError::InvalidVarintPrefix(b) => write!(f, "Invalid varint prefix: {:02X}", b),
            KuError::UnknownGeneType(g) => write!(f, "Unknown gene type: {}", g),
            KuError::PayloadTruncated { expected, got } => write!(f, "Payload truncated: expected {}, got {}", expected, got),
            KuError::PayloadTooLargeV5 { size } => write!(f, "Payload too large for v5: {} bytes (max {})", size, u32::MAX),
