...
use crate::encoder::{
    encode_knowledge_unit, size_breakdown_full,
};
...
fn demo_e2e_fact_creation() {
    ...
    println!("[Step 1] Registry initialized with {} concepts", registry.len());
    ...
    println!("  water       → CID {} {} varint={:02X?}",
             water_id, tier_name(water_id), encode_varint(water_id).unwrap());
    println!("  boiling_point → CID {} {} varint={:02X?}",
             boiling_id, tier_name(boiling_id), encode_varint(boiling_id).unwrap());
    println!("  temperature → CID {} {} varint={:02X?}",
             temp_id, tier_name(temp_id), encode_varint(temp_id).unwrap());
    ...
    let codons = vec![
        ...
        Codon {
            ...
            qualifiers: vec![
                ...
                Qualifier { key: "val".into(), value: QualifierValue::Integer(100) },
            ],
        },
    ];
    ...
    println!("  Magic:   0x{:02X}{:02X} = \"{}{}\"", wire[0], wire[1],
             wire[0] as char, wire[1] as char);
    ...
    println!("[Step 9] ✓ Decoded: gene_type={:?}, CRC valid={}", decoded.gene_type, decoded.crc32_valid);
...
fn demo_e2e_testimony() {
    ...
    println!("Concepts: ufo={} {}, light={} {}, observation={} {}, night_sky={} {}",
             ufo_id, tier_name(ufo_id),
             light_id, tier_name(light_id),
             observation_id, tier_name(observation_id),
             night_sky_id, tier_name(night_sky_id));

    let codons = vec![
        Codon { concept_id: ufo_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: light_id, role: RoleId::Quality, qualifiers: vec![] },
        Codon { concept_id: observation_id, role: RoleId::Object, qualifiers: vec![] },
        Codon { concept_id: night_sky_id, role: RoleId::Location, qualifiers: vec![] },
    ];
    ...
    println!("│ Trust Status:   {:?}", trust_back.epistemic_status);
    println!("│ Trust Score:    {}/10000 ({:.2}%)",
             trust_back.trust_score, trust_back.trust_score as f64 / 100.0);
    println!("│ Error Flags:    0b{:016b}", trust_back.error_susceptibility);
    println!("│ Wire Size:      {} bytes", wire.len());
    println!("│ CRC Valid:      ✓");
    println!("└──────────────────────────────────────────────┘");
...
fn demo_e2e_experience() {
    ...
    println!("Concepts: sunset={}, ocean={}, wonder={}, color={}, horizon={}, warmth={}",
             sunset_id, ocean_id, wonder_id, color_id, horizon_id, warmth_id);

    // Scene codons — sensory experience
    let scene = vec![
        Codon { concept_id: sunset_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: ocean_id, role: RoleId::Location, qualifiers: vec![] },
        Codon {
            concept_id: color_id,
            role: RoleId::Quality,
            qualifiers: vec![
                Qualifier { key: "hue".into(), value: QualifierValue::Text("orange-pink".into()) },
            ],
        },
        Codon { concept_id: horizon_id, role: RoleId::Location, qualifiers: vec![] },
        Codon { concept_id: warmth_id, role: RoleId::Quality, qualifiers: vec![] },
    ];
    ...
    let gene = Gene::Experience {
        ...
        canonical: Some(CanonicalText {
            lang: 1, // English
            text: b"Watching the sun melt into the ocean, painting the sky in orange and pink".to_vec(),
        }),
    ...
    let codons = vec![
        Codon { concept_id: sunset_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: ocean_id, role: RoleId::Location, qualifiers: vec![] },
        Codon { concept_id: wonder_id, role: RoleId::Quality, qualifiers: vec![] },
    ];
    ...
    // Verify affect data
    if let Gene::Experience { ref affect, ref canonical, ref perspective, .. } = ku_back.gene {
        println!("\n┌─── Experience KU ────────────────────────────┐");
        println!("│ Gene Type:      Experience");
        println!("│ VAD Affect:     V={} A={} D={}", affect.v, affect.a, affect.d);
        println!("│   Valence:      {:.2} (positive)", affect.v as f64 / 10000.0);
        println!("│   Arousal:      {:.2} (calm)", affect.a as f64 / 10000.0);
        println!("│   Dominance:    {:.2} (in control)", affect.d as f64 / 10000.0);
        if let Some(ref c) = canonical {
            println!("│ Canonical:      \"{}\"",
                     String::from_utf8_lossy(&c.text));
        }
        if let Some(ref p) = perspective {
            println!("│ Perspective:    type={} expertise={}",
                     match p.perspective_type { 0 => "OBJECTIVE", 1 => "SUBJECTIVE", 2 => "INTERSUBJECTIVE", _ => "CONTESTED" },
                     p.expertise);
        }
        println!("│ Wire Size:      {} bytes", wire.len());
        println!("└──────────────────────────────────────────────┘");
...
fn demo_e2e_formal() {
    ...
    println!("Concepts: energy={}, mass={}, light_speed={}, equation={}",
             energy_id, mass_id, speed_id, equation_id);

    let codons = vec![
        Codon { concept_id: energy_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: mass_id, role: RoleId::Object, qualifiers: vec![] },
        Codon { concept_id: speed_id, role: RoleId::Quality, qualifiers: vec![
            Qualifier { key: "power".into(), value: QualifierValue::Integer(2) },
        ]},
        Codon { concept_id: equation_id, role: RoleId::CompoundHead, qualifiers: vec![] },
    ];
    ...
    if let Gene::Formal { ref notation_source, domain, verification_status, .. } = ku_back.gene {
        println!("\n┌─── Formal KU ────────────────────────────────┐");
        println!("│ Gene Type:      Formal (PHYSICS)");
        println!("│ Notation:       \"{}\"", String::from_utf8_lossy(notation_source));
        println!("│ Domain:         {} (PHYSICS)", domain);
        println!("│ Verification:   {} (FORMALLY_PROVED)", verification_status);
        let trust_back = ku_back.trust.as_ref().unwrap();
        println!("│ Trust Score:    {}/10000 (PERFECT)", trust_back.trust_score);
...
fn demo_e2e_hypothesis() {
    ...
    println!("Concepts: dark_matter={}, galaxy={}, rotation={}",
             dark_matter_id, galaxy_id, rotation_id);

    let codons = vec![
        Codon { concept_id: dark_matter_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: galaxy_id, role: RoleId::Object, qualifiers: vec![] },
        Codon { concept_id: rotation_id, role: RoleId::Result, qualifiers: vec![] },
    ];

    let body_codons = vec![
        Codon { concept_id: dark_matter_id, role: RoleId::Cause, qualifiers: vec![] },
        Codon { concept_id: rotation_id, role: RoleId::Result, qualifiers: vec![] },
    ];
    ...
    println!("Wire encoding: base={} (EXTENDED), ext=0x{:02X}", base, ext.unwrap());
    ...
    assert_eq!(gene_base_from_wire, 7, "FLAGS gene type must be 7 (EXTENDED)");
    ...
    if let Gene::Hypothesis { maturity_level, confidence, falsifiable, .. } = &ku_back.gene {
        println!("\n┌─── Hypothesis KU ────────────────────────────┐");
        println!("│ Gene Type:      Hypothesis (EXTENDED 0x00)");
        println!("│ Maturity:       {} (WELL_SUPPORTED)", maturity_level);
        println!("│ Confidence:     {}/10000 ({:.2}%)", confidence, *confidence as f64 / 100.0);
        ...
        println!("│ Wire Size:      {} bytes", wire.len());
        println!("│ FLAGS byte:     0b{:08b} (gene bits=111=EXTENDED)", flags_byte);
        println!("└──────────────────────────────────────────────┘");
    } else {
...
fn demo_e2e_multi_ku_bonds() {
    ...
    // ── KU1: Original fact ──
    let ku1_codons = vec![
        Codon { concept_id: water_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: boiling_id, role: RoleId::Quality, qualifiers: vec![] },
        Codon { concept_id: temp_id, role: RoleId::Quantity, qualifiers: vec![
            Qualifier { key: "val".into(), value: QualifierValue::Integer(100) },
        ]},
    ];
    let ku1_gene = Gene::Fact {
        triples: vec![Triple { subject: water_id, predicate: boiling_id, object: temp_id }],
        certainty: 9000,
        evidence: vec![],
    };
    ...
    let ku1_fake_cid: Vec<u8> = vec![0xDE, 0xAD, 0xBE, 0xEF,
                                      0x01, 0x02, 0x03, 0x04,
                                      0x05, 0x06, 0x07, 0x08,
                                      0x09, 0x0A, 0x0B, 0x0C,
                                      0x0D, 0x0E, 0x0F, 0x10,
    ...
    let ku2_codons = vec![
        Codon { concept_id: experiment_id, role: RoleId::Agent, qualifiers: vec![] },
        Codon { concept_id: confirmation_id, role: RoleId::Result, qualifiers: vec![] },
        Codon { concept_id: water_id, role: RoleId::Object, qualifiers: vec![] },
    ];
    ...
    let ku2_gene = Gene::Fact {
        triples: vec![Triple { subject: experiment_id, predicate: confirmation_id, object: water_id }],
        certainty: 9500,
        evidence: vec![],
    };
    ...
    println!("│ Relation:       {:?} (category {})",
             bond_back.relation, bond_back.relation.category());
    println!("│ Weight:         {}/10000 ({:.2})",
             bond_back.weight, bond_back.weight as f64 / 10000.0);
    ...
    println!("│ Bond overhead:  ~{} bytes", ku2_wire.len() as i64 - ku1_wire.len() as i64);
    println!("└──────────────────────────────────────────────┘");
...
fn demo_e2e_size_scaling() {
    ...
    println!("│                                              │");
    println!("│  Level 1 (Minimal, no trust):    {:>4} bytes │", wire1.len());
    println!("│  Level 2 (With trust section):   {:>4} bytes │", wire2.len());
    println!("│  Level 3 (Trust + epigenetic):   {:>4} bytes │", wire3.len());
    println!("│                                              │");
    println!("│  Trust overhead:       +{:>4} bytes ({:.1}x)   │",
             wire2.len() - wire1.len(),
             wire2.len() as f64 / wire1.len() as f64);
    println!("│  Epigenetic overhead:  +{:>4} bytes ({:.1}x)   │",
             wire3.len() - wire2.len(),
             wire3.len() as f64 / wire2.len() as f64);
    println!("│  Total overhead:       +{:>4} bytes ({:.1}x)   │",
             wire3.len() - wire1.len(),
    ...
    assert!(wire3.len() > wire1.len() + 600, "Embedding should add ~640+ bytes");
...
fn demo_print_summary() {
    ...
    let registry = ConceptRegistry::new_with_bootstrap();
    println!("║                                                        ║");
    println!("║  📚 Registry: {} concepts bootstrapped                 ║", registry.len());
    ...
    println!("║     Tier 0 (1B, 0-127):       {:>2} concepts             ║", tier_counts[0]);
    println!("║     Tier 1 (2B, 128-16K):     {:>2} concepts             ║", tier_counts[1]);
    println!("║     Tier 2 (3B, 16K-2M):      {:>2} concepts             ║", tier_counts[2]);
    println!("║     Tier 3 (5B, 2M+):         {:>2} concepts             ║", tier_counts[3]);
    ...
    let fact_gene = Gene::Fact {
        triples: vec![Triple { subject: water_id, predicate: boiling_id, object: temp_id }],
        certainty: 9800, evidence: vec![],
    };
    let fact_trust = TrustSection {
        epistemic_status: EpistemicStatus::Evidence,
        evidence_type: EvidenceType::Experimental,
        verification_level: 3, corroboration_count: 42,
        challenge_count: 0, error_susceptibility: 0,
        trust_score: 9500, confidence: 8500,
        domain_codes: vec![140, 141],
        verifications: vec![], challenges: vec![],
        ..Default::default()
    };
    ...
    let tst_ku = build_ku(
        vec![Codon { concept_id: ufo_id, role: RoleId::Agent, qualifiers: vec![] }],
        vec![],
        Gene::Testimony {
            triples: vec![Triple { subject: ufo_id, predicate: 11, object: 203 }],
            claim_type: 0, extraordinary: 2, witness_count: 1, proximity: 0, verification_status: 0,
        },
        Some(TrustSection { epistemic_status: EpistemicStatus::Hearsay,
            evidence_type: EvidenceType::Anecdotal,
            verification_level: 0, corroboration_count: 0, challenge_count: 0,
            error_susceptibility: 0, trust_score: 2000, confidence: 3000,
    ...
    let formal_ku = build_ku(
        vec![Codon { concept_id: 190, role: RoleId::Agent, qualifiers: vec![] }],
        vec![],
        Gene::Formal {
            domain: 1, notation_format: 0, notation_source: b"E=mc^2".to_vec(),
            statement_type: 2, verification_status: 3,
        },
        Some(TrustSection { epistemic_status: EpistemicStatus::FormallyProven,
            evidence_type: EvidenceType::FormalProof,
            verification_level: 4, corroboration_count: 10000, challenge_count: 0,
            error_susceptibility: 0, trust_score: 10000, confidence: 10000,
            domain_codes: vec![140], verifications: vec![], challenges: vec![],
            ..Default::default()
        }),
        None,
    );
    ...
    // Hypothesis (EXTENDED)
    let hyp_ku = build_ku(
        vec![Codon { concept_id: 202, role: RoleId::Agent, qualifiers: vec![] }],
        vec![],
        Gene::Hypothesis {
            base_type: 0, body_codons: vec![
                Codon { concept_id: 202, role: RoleId::Cause, qualifiers: vec![] },
            ],
            maturity_level: 3, confidence: 7000, completeness: 5000, falsifiable: true,
        },
        Some(TrustSection { epistemic_status: EpistemicStatus::Hypothesis,
            evidence_type: EvidenceType::Observational,
            verification_level: 2, corroboration_count: 15, challenge_count: 3,
            error_susceptibility: 0, trust_score: 6000, confidence: 5000,
            domain_codes: vec![140], verifications: vec![], challenges: vec![],
            ..Default::default()
        }),
        None,
    ...
    println!("║     Fact (water boils):        {:>4} bytes              ║", fact_wire.len());
    println!("║     Testimony (UFO):           {:>4} bytes              ║", tst_wire.len());
    println!("║     Formal (E=mc²):            {:>4} bytes              ║", formal_wire.len());
    println!("║     Hypothesis (dark matter):  {:>4} bytes              ║", hyp_wire.len());
    ...
    for (name, wire) in &scenarios {
        match decode_full_knowledge_unit(wire) {
            Ok((decoded, _)) => {
                if !decoded.crc32_valid { all_ok = false; }
            }
            Err(e) => {
    ...
    println!("║                                                        ║");
    println!("║  🔄 Codec Roundtrip: {} success rate           ║",
