...
pub fn encode_codon(codon: &Codon) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(codon, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_codons(codons: &[Codon]) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(&codons, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_bond(bond: &Bond) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(bond, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_gene(gene: &Gene) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(gene, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_trust(trust: &TrustSection) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(trust, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_epigenetic(epigenetic: &EpigeneticSection) -> Result<Vec<u8>, KuError> {
    let mut buf = Vec::new();
    ciborium::into_writer(epigenetic, &mut buf).map_err(|e| KuError::CborEncode(e.to_string()))?;
    Ok(buf)
}
...
pub fn encode_knowledge_unit(ku: &KnowledgeUnit) -> Result<Vec<u8>, KuError> {
    ...
    if payload.len() > u32::MAX as usize {
        return Err(KuError::PayloadTooLargeV5 {
            size: payload.len(),
        });
    }
...
pub fn print_size_breakdown(ku: &KnowledgeUnit) {
    let codons_size = if let Ok(b) = encode_codons(&ku.codons) {
        b.len()
    } else {
        return;
    };
    let bonds_sizes: Vec<usize> = ku
        .bonds
        .iter()
        .filter_map(|b| encode_bond(b).ok().map(|v| v.len()))
        .collect();
    let gene_size = if let Ok(b) = encode_gene(&ku.gene) {
        b.len()
    } else {
        return;
    };
    let wire = if let Ok(w) = encode_knowledge_unit(ku) {
        w
    } else {
        return;
    };
    ...
    println!(
        "  Layer 2 total:    {} bytes ({} bonds)",
        bonds_sizes.iter().sum::<usize>(),
        ku.bonds.len()
    );
