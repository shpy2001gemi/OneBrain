...
pub async fn run_seed_server(bind_addr: SocketAddr, name: &str, max_peers: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if !removed.is_empty() {
                println!("  [cleanup] Removed {} stale peer(s). Active: {}", removed.len(), s.registry.peer_count());
            }
        }
    });
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            println!("  [stats] Peers: {} registered, {} connected", 
                s.registry.peer_count(), s.relay.connection_count());
        }
    });
...
async fn handle_client(stream: tokio::net::TcpStream, addr: SocketAddr, state: Arc<Mutex<SeedState>>) {
    ...
    loop {
        ...
        match msg {
            SeedMessage::Register { peer_id, name, listen_port: _, internal_addr, upnp_addr, ku_count } => {
                let mut s = state.lock().await;
                let registered = s.registry.register(
                    peer_id.clone(), name.clone(), addr, internal_addr, upnp_addr, ku_count,
                );
                if registered {
                    ...
                    let mut guard = stream.lock().await;
                    let _ = send_message(&mut *guard, &SeedMessage::SeedError {
            ...
            SeedMessage::RelayToPeer { to_peer_id, payload } => {
                let s = state.lock().await;
                if let Some(pid) = &peer_id_opt {
                    let from_name = s.registry.get_peer(pid)
                        .map(|r| r.name.clone())
                        .unwrap_or_else(|| "unknown".to_string());
                    let _ = s.relay.relay_to(&to_peer_id, pid, &from_name, payload).await;
                }
            }
