...
impl BlobStorage {
    ...
    fn write_chunks(&self, blob_cid: &BlobCid, data: &[u8], chunk_count: u32, mode: StorageMode) -> Result<(), BlobStorageError> {
        match mode {
            StorageMode::Redb => {
                let txn = self.db.begin_write()?;
                {
    ...
    fn read_chunk_internal(&self, blob_cid: &BlobCid, index: u32, mode: StorageMode) -> Result<Vec<u8>, BlobStorageError> {
        match mode {
            StorageMode::Redb => {
                let mut key = Vec::with_capacity(38);
                key.extend_from_slice(&blob_cid.0);
            ...
            StorageMode::Filesystem => {
                let chunk_path = self.blob_chunk_dir(blob_cid)
    ...
    fn delete_chunks_internal(&self, blob_cid: &BlobCid, chunk_count: u32, mode: StorageMode) -> Result<(), BlobStorageError> {
        match mode {
            StorageMode::Redb => {
                let txn = self.db.begin_write()?;
                {
...
mod tests {
    ...
    fn small_blob_uses_redb_mode() {
        ...
        assert!(!chunk_dir.exists(), "Small blob should not create filesystem chunk dir");
    }
    ...
    fn large_blob_uses_filesystem_mode() {
        ...
        let cid = BlobCid::from_hex(&meta.blob_cid_hex).unwrap();
        let chunk_dir = storage.blob_chunk_dir(&cid);
        assert!(chunk_dir.exists(), "Large blob should create filesystem chunk dir");
        ...
        assert!(!chunk_dir.exists(), "Chunk dir should be deleted after blob deletion");
    ...
    fn large_blob_store_bytes() {
        ...
        let meta = storage.store_bytes("big.dat", &content, BlobType::Raw).unwrap();
